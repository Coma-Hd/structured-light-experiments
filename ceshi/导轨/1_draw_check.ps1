...
$RoiFile = "$WorkDir/roi_keyframes.json"
...
if ($ReuseRoi) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Curve keyframe ROI selection failed."
    }
}
...
Write-Host "The interpolated ROI must cover the complete object laser in every check image."
Write-Host "Red centers must follow the laser core and avoid the board/background."
